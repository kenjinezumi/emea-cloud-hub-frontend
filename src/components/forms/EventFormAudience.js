...
export default function AudiencePersonaForm() {
  ...
  const handleCheckboxChange = (event) => {
    setAccountSegments({ ...accountSegments, [event.target.name]: event.target.checked });
  };
  ...
  const handleNext = () => {
    ...
    const isAccountSegmentsSelected = Object.values(accountSegments).some(Boolean);

    const formIsValid = isAudiencePersonaValid && isAudienceSeniorityValid && isAccountSectorsValid && isMaxEventCapacityValid && isPeopleMeetingCriteriaValid && isAccountSegmentsSelected;
      ...
      <Grid container spacing={2}>
          ...
          <Select
            ...
            >
              {audienceSeniorityOptions.map((seniority) => (
                <MenuItem key={seniority} value={seniority}>{seniority}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Grid>
        ...
        <Grid item xs={12}>
            <FormControl component="fieldset">
              ...
              >
                <FormControlLabel value="commercial sector" control={<Radio />} label="Commercial sector" />
                <Grid item xs={12} />
                <FormControlLabel value="public sector" control={<Radio />} label="Public sector" />
              </RadioGroup>
            </FormControl>
          </Grid>
        ...
        <Typography variant="subtitle1">
          ...
          <FormGroup>
            <FormControlLabel
              control={<Checkbox checked={accountSegments.segment1} onChange={handleCheckboxChange} name="enterprise" />}
              label="Enterprise"
            />
            <FormControlLabel
              control={<Checkbox checked={accountSegments.segment2} onChange={handleCheckboxChange} name="corporate" />}
              label="Corporate"
            />
             <FormControlLabel
              control={<Checkbox checked={accountSegments.segment3} onChange={handleCheckboxChange} name="smb" />}
              label="SMB"
            />
